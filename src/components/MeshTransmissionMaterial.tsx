    ...
    useFrame((state) => {
      ...
      if (ref.current.buffer === fboMain.texture && !transmissionSampler) {
        ...
        if (parent) {
          ...
          // Preserve the original material reference so a multi-material mesh
          // (using attach="material-X") still has its other slots intact after
          // the off-screen FBO pass.
          ...
          if (backside) {
            ...
            // Prepare uniforms on this material for the main render using the backside buffer.
            // Restore the saved material array (so the next render uses it again
            // for other groups), and write the buffer/thickness/side/envMapIntensity
            // onto this material directly instead of the parent's `.material` slot
            // — that would be the whole array when multi-material is in use.
